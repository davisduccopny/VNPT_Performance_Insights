...
class ORTHER_FUNCTION_INSERT():
    def __init__(self):
        pass
    def get_data_from_kehoach_for_view(self,kehoach_after_load_dl,line_delete, selected_year_delete, search_term=None):
        kehoach_after_filter = kehoach_after_load_dl[(kehoach_after_load_dl["year_insert"] == selected_year_delete) & 
                                                      (kehoach_after_load_dl["line"] == line_delete)].reset_index(drop=True)
        unique_kehoach_for_delete = kehoach_after_load_dl[(kehoach_after_load_dl["year_insert"] == selected_year_delete) & 
                                  (kehoach_after_load_dl["line"] == line_delete)][["year_insert", "line", "loaidoanhthu"]].drop_duplicates(subset=["year_insert", "line", "loaidoanhthu"]).reset_index(drop=True)
        unique_kehoach_for_delete["ten_line"] = line_after_load_dl.loc[line_after_load_dl["ma_line"] == line_delete, "ten_line"].values[0] if not line_after_load_dl.loc[line_after_load_dl["ma_line"] == line_delete, "ten_line"].empty else None
        if search_term:
    ...
    def get_data_from_thuchien_for_view(self,thuchien_after_load_dl,line_delete, selected_year_delete, selected_loaidoanhthu_delete, selected_month_delete, search_term=None):
        thuchien_after_filter = thuchien_after_load_dl[(thuchien_after_load_dl["year_insert"] == selected_year_delete) &
                                                        (thuchien_after_load_dl["line"] == line_delete) &
        ...
        unique_thuchien_for_delete = thuchien_after_load_dl[(thuchien_after_load_dl["year_insert"] == selected_year_delete) &
                                                            (thuchien_after_load_dl["line"] == line_delete) &
                                                            (thuchien_after_load_dl["loaidoanhthu"] == selected_loaidoanhthu_delete)][["year_insert","thang", "line", "loaidoanhthu"]].drop_duplicates(subset=["year_insert","thang", "line", "loaidoanhthu"]).reset_index(drop=True)
        unique_thuchien_for_delete["ten_line"] = line_after_load_dl.loc[line_after_load_dl["ma_line"] == line_delete, "ten_line"].values[0] if not line_after_load_dl.loc[line_after_load_dl["ma_line"] == line_delete, "ten_line"].empty else None
        if search_term:
...
class FRONTEND_DESIGN_DELETE():
    ...
    def ui_sidebar_option(self, selected):
        ...
        if selected == "Kế hoạch":
            ...
            selected_loaidoanhthu_delete = st.sidebar.selectbox("Chọn loại doanh thu", thuchien_after_load_dl["loaidoanhthu"].unique(), key="selected_loaidoanhthu_delete")
            selected_month_delete = st.sidebar.selectbox("Chọn tháng",thuchien_after_load_dl["thang"].unique(), key="selected_month_delete")
        return selected_year_delete, selected_loaidoanhthu_delete, selected_month_delete, line_delete
    def delete_kehoach(self,selected_year_delete,line_delete, search_term):
        kehoach_after_filter, unique_kehoach_for_delete = ORTHER_FUNCTION_INSERT().get_data_from_kehoach_for_view(kehoach_after_load_dl,line_delete, selected_year_delete, search_term)
        ...
        button_delete_kehoach = cols_plan_delete[3].button("Xóa khỏi csdl", key="delete_plan", type="primary",icon=":material/delete:",use_container_width=True)    
    ...
    def delete_thuchien(self, thuchien_after_load_dl,line_delete, selected_year_delete, selected_loaidoanhthu_delete, selected_month_delete, search_term):
        thuchien_after_filter, unique_thuchien_for_delete = ORTHER_FUNCTION_INSERT().get_data_from_thuchien_for_view(thuchien_after_load_dl,line_delete, selected_year_delete, selected_loaidoanhthu_delete, selected_month_delete, search_term)
        ...
        with container_header_do_delete:
            ...
            button_delete_do = cols_do_delete[3].button("Xóa khỏi csdl", key="delete_do_out_db", type="primary",icon=":material/delete:",use_container_width=True)
                    ...
                    with tabs_do_delete[0]:
                        delete_data = st.data_editor(unique_thuchien_for_delete, num_rows="fixed",
                                                        column_config={"year_insert": "Năm",
                                                                    ...
                                                                    "loaidoanhthu": "Loại doanh thu",
                                                                    "Delete": "Chọn xóa"}, 
                                                        disabled=(("year_insert","thang","line","loaidoanhthu")), use_container_width=True,
                                                        hide_index=True)
                        deleted_rows = delete_data[delete_data['Delete']].to_dict('records')
                        deleted_data_delete = [(row['thang'],row['year_insert'],row['line'],row['loaidoanhthu']) for row in deleted_rows]
                        if button_delete_do:
                            if deleted_rows == [] or deleted_rows == None:
                                st.toast("##### Bạn chưa chọn dữ liệu để xóa!", icon="⚠️")
                                time.sleep(2)
...
class MAIN_APP_DELETE():
    ...
    def main(self, selected):
        ...
        if selected == "Kế hoạch":
            ...
            selected_year_delete, selected_loaidoanhthu_delete, selected_month_delete, line_delete = ui.ui_sidebar_option(selected)
            self.search_term = ui.ui_info(text="XÓA DỮ LIỆU - 📝KẾ HOẠCH",loai_data=self.loai_data)
            ui.delete_kehoach(selected_year_delete,line_delete, self.search_term)
            ...
            ui.delete_thuchien(thuchien_after_load_dl,line_delete, selected_year_delete, selected_loaidoanhthu_delete, selected_month_delete, self.search_term)

            
        

thuchien_after_load_dl, kehoach_after_load_dl,nhanvien_after_load_dl, dichvu_after_load_dl,line_after_load_dl = module_view.load_data() 
ui = FRONTEND_DESIGN_DELETE()
