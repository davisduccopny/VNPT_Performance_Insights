...
class FRONTEND_DESIGN_INSERT():
    ...
    def ui_make_project_insert(self):
        ...
        with container_make_header:
            ...
            with col_make_project_header[1]:
                with st.popover("Xem data", icon="👁️‍🗨️", help="🔍Xem data đang tồn tại trong csdl.", use_container_width=True):
                    radio_preview_make_project = st.radio(label="Chọn loại doanh thu", options=["Hiện hữu","Phát triển mới"], key="radio_preview_make_project",horizontal=True)
        ...
        col_project_insert_1,col_project_insert_2,col_project_insert_3 = st.columns(3)
                    ...
                    for j in range(columns_per_row):
                        if i + j < len(docs):
                            ...
                            with cols[j]:
                                ...
                                st.markdown(f"""<img src="{image_file_path}" style="height:10vh;"></img>""", unsafe_allow_html=True)
