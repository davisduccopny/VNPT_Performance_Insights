...
def select_rows_kehoach_for_insert(data, result_service,form_service_selected_insert):
    """
    Lọc các dòng từ DataFrame `data` với điều kiện giá trị trong cột `id_dv_606` nằm trong `nhom_dv_id`.
    """
    data = data[['id_dv_606', 't1', 't2', 't3', 't4', 't5', 't6', 't7', 't8', 't9', 't10', 't11', 't12']]
    ...
    if form_service_selected_insert == "Thu gọn":
    ...
    subset.iloc[:, 1:] = subset.iloc[:, 1:].fillna(0)
    return subset

def insertData_kehoach_to_database(radiotype,line, year,loaidoanhthu, data,conn):
...
# Lặp qua từng hàng trong DataFrame
    try:
        for index, row in data.iterrows():
            ...
            sql = """INSERT INTO kehoach_linelv
                    (id_dv_606,t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12,line,year_insert,loaidoanhthu, type_process)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
            # Dữ liệu để insert, kết hợp với line và year
            data_to_insert = tuple(list(row[:14]) + [line, year,loaidoanhthu,radiotype])
...
def query_kehoach_by_line_year(radiotype,line_test, year_test,loaidoanhthu,conn):
    cursor = conn.cursor()

    # Thực hiện truy vấn SQL
    cursor.execute("SELECT * FROM kehoach_linelv WHERE line=%s AND year_insert=%s AND loaidoanhthu=%s AND type_process =%s;", (line_test, year_test,loaidoanhthu,radiotype))
    rows = cursor.fetchall()
...
def validate_columns(data):
    required_columns = ["id_dv_606", "t1", "t2", "t3", "t4", "t5", "t6", "t7", "t8", "t9", "t10", "t11", "t12"]
    ...
    if missing_columns:
        ...
        return False
# PART 2: THUCHIEN
def get_data_thuchien(radio_type_process,thang, nam,loaidoanhthu,conn):
    cursor = conn.cursor()
    query = "SELECT nhom_dv, doanhthu, thang, year_insert, loaidoanhthu,line FROM thuchien WHERE thang=%s AND year_insert=%s AND loaidoanhthu=%s AND type_process=%s;"
    cursor.execute(query, (thang, nam, loaidoanhthu, radio_type_process))
    ...
    # Trả về DataFrame
    return kq_thuchien
def group_data_for_insert_thuchien(data,type_process):
    if type_process != "LDPVNPT":
        grouped_data = data.groupby(['MA_NV','TEN_TO_CS', 'NHOM DV']).agg({'DOANH THU': 'sum'}).reset_index()
    else:
        grouped_data = data.groupby(['TEN_TO_CS', 'NHOM DV']).agg({'DOANH THU': 'sum'}).reset_index()
    return grouped_data
def validate_thuchien_columns(data,type_process):
    required_columns = ["MA_NV", "NHOM DV","DOANH THU","TEN_TO_CS"] if type_process != "LDPVNPT" else ["TEN_TO_CS", "NHOM DV","DOANH THU"]
    missing_columns = [col for col in required_columns if col not in data.columns]
    if missing_columns:
        return missing_columns
    else:
        return False
def insert_data_to_thuchien(radio_type_process,thang, nam,loaidoanhthu, group_data,conn):
    # Tạo một con trỏ để thực thi các câu lệnh SQL
    cursor = conn.cursor()
    ...
    for index, row in group_data.iterrows():
        ...
        if radio_type_process != "LDPVNPT":
            data_to_insert = (row.iloc[0], row.iloc[2], row.iloc[3],row.iloc[1], thang, nam,loaidoanhthu, radio_type_process)
        else:
            data_to_insert = (radio_type_process, row.iloc[1], row.iloc[2],row.iloc[0], thang, nam,loaidoanhthu, radio_type_process)
    ...
    if cursor.rowcount > 0:
        ...
        cursor.close()
        return False
def get_data_preview_for_insert_database(radio_type_process,year,loaidoanhthu,conn):
    cursor = conn.cursor()
    query = "SELECT DISTINCT(thang),line,year_insert,loaidoanhthu FROM thuchien WHERE year_insert =%s AND loaidoanhthu=%s AND type_process=%s;"
    cursor.execute(query, (year,loaidoanhthu,radio_type_process))
